...
- vendor_job_number: the vendor's OWN internal job/project number for this work — the reference they use to track this project in their own system. This is different from the invoice number. It often appears as "Project No.", "Job No.", "Project #", "Our Job", or similar. Extract only the number/code itself (e.g. "24117"). Empty string if not present.
...
const INVOICE_SCHEMA = {
  ...
  required: ['invoice_number','invoice_number_confidence','vendor_name','vendor_name_confidence',
             ...
             'services_thru_date','summary','line_items','project_clues','vendor_job_number'],
  additionalProperties: false,
};
...
async function extractInvoice(pdfBuffer, context = {}) {
  ...
  return {
    ...
    vendor_job_number: String(data.vendor_job_number || '').trim(),
    usage,
  };
}
...
1. The invoice PDF
2. The vendor name
3. A numbered list of budget tasks already defined for this project (task ID, task name, discipline, and the GL account that task maps to)
4. A numbered list of all available QB accounts as a fallback (ID, account_number, full_name)
5. The extracted line items (index, description, amount)

STEP 1 — MATCH TO BUDGET TASK (primary signal):
For each line item, find the budget task whose name and discipline most closely matches the line description. The GL code for that task is the correct answer. This is always preferred over guessing by vendor type.

STEP 2 — FALLBACK TO QB ACCOUNTS LIST:
Only if no budget task is a reasonable match, choose the best QB account from the fallback list.

CRITICAL RULE: The vendor's trade does NOT determine the GL code. A civil engineering firm can bill for fire suppression reports, traffic studies, environmental work, or survey — each of those is a different GL code based on the WORK TYPE, not the vendor type. Always read the line description carefully.
...
- qb_account_id: integer ID from the provided QB accounts list (must be exact)
- account_number: the account number string (e.g. "1720.10")
- confidence: "high" | "medium" | "low"
- reason: one short sentence — if matched via budget task, name the task; if fallback, explain why

Rules:
- Only use IDs from the provided QB accounts list — never invent codes
- High confidence = clear task name match; Medium = partial match; Low = fallback/guess`;
...
async function suggestInvoiceLineCodes(pdfBuffer, lines, qbAccounts, vendorName, budgetLines = []) {
  if (!qbAccounts || qbAccounts.length === 0 || !lines || lines.length === 0) return [];

  // Build a GL code lookup from budget lines so we can include the account number in the task list
  const glById = {};
  for (const a of qbAccounts) glById[a.id] = a;

  const taskList = budgetLines.length
    ? budgetLines.map(bl => {
        const gl = glById[bl.qb_account_id];
        return `Task "${bl.task_name}"${bl.discipline ? ` (${bl.discipline})` : ''} → GL account ID ${gl?.id ?? '?'}: [${gl?.account_number ?? '?'}] ${gl?.full_name ?? ''}`;
      }).join('\n')
    : '(no budget tasks available for this phase)';
...
BUDGET TASKS FOR THIS PROJECT (match line descriptions against these first):
${taskList}

FALLBACK — ALL QB ACCOUNTS (use only if no budget task matches):
${accountList}

Line items to code:
${lineList}

Suggest the best QB account for each line. Prefer matching via budget task name.`
  );
...
// matchProject: check filename + extracted clues + vendor job number against a project.
// allProjects: array of { name, keywords, id } for ALL projects.
// vendorJobMap: array of { vendor_name, vendor_job_number, project_id } from vendor_project_map table.
function matchProject(projectClues, projectName, projectAliases = [], filename = '', allProjects = [], vendorJobMap = [], vendorName = '', vendorJobNumber = '', currentProjectId = null) {
  const haystack = [projectClues || '', filename].join(' ').toLowerCase();

  // ── Check 1: vendor job number (strongest signal) ─────────────────────────
  if (vendorName && vendorJobNumber) {
    const vn = vendorName.toLowerCase().trim();
    const jn = vendorJobNumber.toLowerCase().trim();
    const known = vendorJobMap.filter(m =>
      m.vendor_name.toLowerCase().trim() === vn &&
      m.vendor_job_number.toLowerCase().trim() === jn
    );
    if (known.length > 0) {
      const matchesThis = known.some(m => m.project_id === currentProjectId);
      if (matchesThis) {
        return { match: 'confirmed', reason: `Vendor job # ${vendorJobNumber} is on record for this project` };
      } else {
        const other = allProjects.find(p => p.id === known[0].project_id);
        return { match: 'mismatch', reason: `Vendor job # ${vendorJobNumber} is on record for project "${other?.name || 'another project'}", not "${projectName}"` };
      }
    }
  }

  if (!haystack.trim()) return { match: 'unknown', reason: 'No project references found' };

  const allNames = [projectName, ...projectAliases].map(n => n.toLowerCase());

  function hitsNames(names) {
    for (const name of names) {
      const words = name.split(/[\s\-\/]+/).filter(w => w.length >= 4);
      const hits = words.filter(w => haystack.includes(w));
      if (hits.length >= 2 || (hits.length === 1 && words.length <= 2)) return hits;
    }
    return null;
  }

  // ── Check 2: filename / clues keyword match ───────────────────────────────
  const thisHit = hitsNames(allNames);
  if (thisHit) {
    return { match: 'confirmed', reason: `Filename/clues mention "${thisHit.join(', ')}" matching project "${projectName}"` };
  }

  // ── Check 3: clues match a DIFFERENT known project → mismatch ────────────
  for (const other of allProjects) {
    if (other.name.toLowerCase() === projectName.toLowerCase()) continue;
    const otherNames = [other.name, ...(other.keywords || [])].map(n => n.toLowerCase());
    const otherHit = hitsNames(otherNames);
    if (otherHit) {
      return { match: 'mismatch', reason: `Filename/clues suggest project "${other.name}" (matched "${otherHit.join(', ')}"), not "${projectName}"` };
    }
  }

  if (haystack.trim().length > 0) {
    return { match: 'uncertain', reason: `Has project clues but no clear match to "${projectName}"` };
  }
  return { match: 'unknown', reason: 'No recognizable project references' };
}
