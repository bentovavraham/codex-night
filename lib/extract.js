...
const SCHEMA = {
  ...
  properties: {
    ...
    invoice_date:   { type: 'string', description: "Invoice date in YYYY-MM-DD format, or empty string if not found." },
...
async function extractInvoice(pdfBuffer) {
  ...
  try {
    ...
    // Surface Anthropic's actual error body so the failure is diagnosable.
    const detail =
  ...
  // Light normalization — dates only pass through if they're a real YYYY-MM-DD.
  const raw_date = (extracted.invoice_date || '').trim();
  return {
    invoice_number: String(extracted.invoice_number || '').trim(),
    vendor_name:    String(extracted.vendor_name || '').trim(),
    amount:         Number(extracted.amount) || 0,
    invoice_date:   /^\d{4}-\d{2}-\d{2}$/.test(raw_date) ? raw_date : null,
