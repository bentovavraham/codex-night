...
async function syncInvoiceFA(client, invoiceId, userId) {
  ...
  if (!wroteFA) {
    ...
    if (resolvedGl && Number(inv.amount)) {
      ...
      await client.query(
        ...
        [invoiceId, inv.phase_id, resolvedGl, pblId,
         inv.invoice_type ?? 'fixed', Number(inv.amount),
         pblId ? 'confirmed' : 'needs_review', userId]
      );
    }
  }
...
module.exports = { createContract, createInvoice, syncContractFA, syncInvoiceFA, resolvePbl };
